...
ODS_PATH = BASE_DIR / "Dataset_OCR.ods"
OUTPUT_DIR = BASE_DIR
STANZA_DIR = BASE_DIR / "stanza_chg" / "tokenizer"

RANDOM_SEED = 42
TRAIN_RATIO = 0.8
DEV_RATIO_FROM_TRAIN = 0.1
RANDOM_TRAIN_SAMPLES = 200
PARTIAL_MERGE_SAMPLES = 200

# Sentence boundaries come from ODS rows. Punctuation is removed only after
# the row-level sentence boundaries are already known.
PUNCTUATION_RE = re.compile(r"""[.۔!?؟,:;؛،()\[\]{}"“”‘’«»*ـ_/|\-–—№]""")
# Arabic diacritics (harakat/tashkeel) are irrelevant for sentence boundary
# detection and are stripped for consistency with the UZS pipeline.
...
def read_ods_original_sentences(path: Path = ODS_PATH) -> list[str]:
    with ZipFile(path) as archive:
        root = ET.fromstring(archive.read("content.xml"))

    rows: list[list[str]] = []
    for table_row in root.findall(".//table:table-row", ODS_NS):
        values = []
        for cell in table_row.findall("table:table-cell", ODS_NS):
    ...
    if not rows or not rows[0] or rows[0][0] != "Original":
        raise ValueError("Could not find the 'Original' column in Dataset_OCR.ods")

    sentences = [row[0] for row in rows[1:] if row and row[0].strip()]
    sentences = [normalize_spaces(sentence) for sentence in sentences]
    # Drop "sentences" that are pure punctuation/formatting artifacts and
    # would collapse to zero tokens after clean_sentence, which breaks
...
def partial_merge(
    ...
    for _ in range(num_samples):
        ...
        # Remove the beginning of sentence 1 and the end of sentence 2.
        # The original boundary remains between the two retained fragments.
...
def write_outputs(train_df: pd.DataFrame, dev_df: pd.DataFrame, test_df: pd.DataFrame) -> None:
    ...
    test_df.to_csv(OUTPUT_DIR / "test_sequential.csv", index=False, encoding="utf-8-sig")
...
def main() -> None:
    ...
    test_samples = sequential_concatenation(test_source, rng)

    train_df = pd.DataFrame(train_samples)
    dev_df = pd.DataFrame(dev_samples)
    test_df = pd.DataFrame(test_samples)
    write_outputs(train_df, dev_df, test_df)
